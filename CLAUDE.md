...
3. **`identify_http_header_directions_with_model`** — for headers still `UNKNOWN`, asks `openai/gpt-oss-20b` (structured output via `with_structured_output`) to classify direction from a windowed excerpt of the RFC, then retries with as much of the full RFC as fits in the context window if still unresolved.
